...
class EvoAttack():
    def __init__(self, dataset, model, x, y, n_gen=500, pop_size=40, eps=0.3, tournament=35, defense=False, norm='linf',
                 i=None):
        ...
        while not self.termination_condition(cur_pop, gen):
            ...
            if self.norm == 'linf':
                ...
                for i in range(self.pop_size - int(elitism_enabled)):  # we might have already added 1 elitist
                    ...
                    plt_torch_image(ind, self.x, f'Evo {self.i}: Gen {gen}, ind {i}')
